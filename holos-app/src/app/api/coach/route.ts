...
IMPORTANT FORMATTING: After your response, on a new line add exactly:
SUGGESTIONS: [option1] | [option2] | [option3] | [option4]
Each option is a SHORT (4-7 words) reply the user might realistically say next, in the same language as your response.
Make options directly relevant to what you just asked or discussed — NOT generic.
Example: if you asked about sleep difficulties, options could be "Сложно заснуть | Просыпаюсь ночью | Встаю уставшим | Сплю нормально".`
...
// ── Fallback suggestion sets per topic per locale ────────────────────────────
const TOPIC_SUGGESTIONS: Record<string, Partial<Record<string, string[]>>> = {
  ru: {
    sleep:        ['Сложно заснуть',         'Просыпаюсь ночью',         'Встаю уставшим',          'Сплю меньше 7 часов'],
    stress:       ['Давит работа',            'Тревога внутри',           'Проблемы в отношениях',   'Всего понемногу'],
    nutrition:    ['Время приёмов пищи',      'Качество продуктов',       'Переедание',              'Нет аппетита'],
    movement:     ['Нет времени',             'Нет мотивации',            'Физическое состояние',    'Не знаю с чего начать'],
    energy:       ['Устал уже утром',         'Спад после обеда',         'К вечеру нет сил',        'Всегда вялый'],
    emotional:    ['Тревога и беспокойство',  'Трудности в отношениях',   'Ощущение потери смысла',  'Перегрузка'],
    purpose:      ['Да, раньше было ярче',    'Работа кажется бессмысленной', 'Хочу найти цель',    'У меня всё нормально'],
    assessment:   ['Сон',                     'Стресс',                   'Питание',                 'Эмоции'],
    action:       ['Начать со сна',           'Разобраться со стрессом',  'Улучшить питание',        'Больше двигаться'],
    ayurveda:     ['Вата — тревога, сухость', 'Питта — жар, раздражение', 'Капха — вялость, вес',   'Не знаю'],
    rambam:       ['Сон и отдых',             'Питание и пищеварение',    'Движение',                'Душевное состояние'],
    tibetan:      ['Нервозность и тревога',   'Жар и раздражение',        'Тяжесть и вялость',       'Сочетание нескольких'],
    daoist:       ['Застой и тяжесть',        'Рассеянность, нет фокуса', 'Стресс и напряжение',     'Слабость и упадок'],
    hippocrates:  ['Питание',                 'Движение',                 'Окружающая среда',        'Всё понемногу'],
    avicenna:     ['Больше физически',        'Больше душевно',           'И то и другое',           'Затрудняюсь ответить'],
    swarga:       ['Сон',                     'Стресс',                   'Питание',                 'Смысл и цели'],
    general_low:  ['Сон — главная проблема',  'Много стресса',            'Нет энергии',             'Питание и вес'],
    general_mid:  ['Хочу больше энергии',     'Справиться со стрессом',   'Спать лучше',             'Привести питание в порядок'],
    general_high: ['Тонкая настройка',        'Улучшить восстановление',  'Оптимизировать питание',  'Укрепить осознанность'],
  },
  he: {
    sleep:        ['קשה להירדם',              'מתעורר בלילה',             'קם עייף',                 'ישן פחות מ-7 שעות'],
    stress:       ['לחץ בעבודה',              'חרדה פנימית',              'בעיות ביחסים',            'הכל ביחד'],
    nutrition:    ['עיתוי הארוחות',           'איכות המזון',              'אכילת יתר',               'חוסר תיאבון'],
    movement:     ['אין זמן',                 'אין מוטיבציה',             'מצב פיזי',                'לא יודע מאיפה להתחיל'],
    energy:       ['עייף כבר בבוקר',          'ירידה אחרי הצהריים',       'אין כוח בערב',            'תמיד עייף'],
    emotional:    ['חרדה ודאגה',              'קשיים ביחסים',             'חוסר משמעות',             'עומס'],
    purpose:      ['כן, לפני הכל היה חי יותר','העבודה מרגישה חסרת משמעות','רוצה למצוא מטרה',       'אצלי הכל בסדר'],
    assessment:   ['שינה',                    'לחץ',                      'תזונה',                   'רגשות'],
    action:       ['להתחיל עם שינה',          'לטפל בלחץ',                'לשפר תזונה',              'לזוז יותר'],
    general_low:  ['שינה - הבעיה הגדולה',     'הרבה לחץ',                 'אין אנרגיה',              'תזונה ומשקל'],
    general_mid:  ['לשפר אנרגיה',             'לנהל לחץ',                 'לישון טוב יותר',          'לסדר תזונה'],
    general_high: ['כוונון עדין',             'שיפור התאוששות',           'אופטימיזציית תזונה',      'חיזוק מיינדפולנס'],
  },
  de: {
    sleep:        ['Schwer einzuschlafen',    'Wache nachts auf',          'Stehe erschöpft auf',     'Schlafe unter 7 Stunden'],
    stress:       ['Arbeitsdruck',            'Innere Angst',              'Beziehungsprobleme',      'Alles zusammen'],
    nutrition:    ['Mahlzeiten-Timing',       'Lebensmittelqualität',      'Überessen',               'Kein Appetit'],
    movement:     ['Keine Zeit',              'Keine Motivation',          'Körperlicher Zustand',    'Weiß nicht wo anfangen'],
    energy:       ['Morgens schon müde',      'Nachmittags-Einbruch',      'Abends keine Kraft',      'Immer träge'],
    emotional:    ['Angst und Sorge',         'Beziehungsprobleme',        'Sinnlosigkeit',           'Überlastung'],
    purpose:      ['Ja, früher war es lebendiger','Arbeit fühlt sich sinnlos an','Möchte Ziel finden','Bei mir ist alles gut'],
    assessment:   ['Schlaf',                  'Stress',                    'Ernährung',               'Emotionen'],
    action:       ['Mit Schlaf anfangen',     'Stress angehen',            'Ernährung verbessern',    'Mehr bewegen'],
    general_low:  ['Schlaf ist das Problem',  'Viel Stress',               'Keine Energie',           'Ernährung und Gewicht'],
    general_mid:  ['Energie verbessern',      'Stress bewältigen',         'Besser schlafen',         'Ernährung ordnen'],
    general_high: ['Feinabstimmung',          'Erholung verbessern',       'Ernährung optimieren',    'Achtsamkeit stärken'],
  },
  en: {
    sleep:        ['Hard to fall asleep',     'Wake up during the night',  'Wake up already tired',   'Sleep less than 7 hours'],
    stress:       ['Work pressure',           'Inner anxiety',             'Relationship difficulties','A bit of everything'],
    nutrition:    ['Meal timing',             'Food quality',              'Overeating',              'No appetite'],
    movement:     ['No time',                 'Lack of motivation',        'Physical condition',      "Don't know where to start"],
    energy:       ['Already tired in the morning','Afternoon slump',       'No energy in the evening','Always sluggish'],
    emotional:    ['Anxiety and worry',       'Relationship difficulties', 'Lack of meaning',         'Overwhelm'],
    purpose:      ['Yes, everything felt more alive','Work feels meaningless','I want to find purpose','Everything is fine for me'],
    assessment:   ['Sleep',                   'Stress',                    'Nutrition',               'Emotions'],
    action:       ['Start with sleep',        'Address stress',            'Improve nutrition',       'Move more'],
    ayurveda:     ['Vata — anxiety, dryness', 'Pitta — heat, irritability','Kapha — sluggishness, weight',"I'm not sure"],
    rambam:       ['Sleep and rest',          'Diet and digestion',        'Movement',                'Emotional state'],
    tibetan:      ['Nervousness and anxiety', 'Internal heat, irritability','Heaviness and sluggishness','A combination'],
    daoist:       ['Stagnation and heaviness','Scattered focus',           'Stress and tension',      'Weakness and low energy'],
    hippocrates:  ['Nutrition',               'Movement',                  'Environment',             'A bit of everything'],
    avicenna:     ['More physical',           'More emotional',            'Both equally',            "I'm not sure"],
    swarga:       ['Sleep',                   'Stress',                    'Nutrition',               'Purpose and goals'],
    general_low:  ['Sleep is the main issue', 'Too much stress',           'No energy',               'Nutrition and weight'],
    general_mid:  ['Improve energy',          'Manage stress',             'Sleep better',            'Sort out nutrition'],
    general_high: ['Fine-tuning',             'Improve recovery',          'Optimize nutrition',      'Strengthen mindfulness'],
  },
}

/** Extract SUGGESTIONS line from Claude's raw output. Returns { reply, suggestions }. */
...
function getSmartFallback(message: string, score: number, lang: string): { reply: string; suggestions: string[] } {
  ...
  for (const { keywords, topic } of patterns) {
    if (keywords.some(kw => m.includes(kw))) {
      ...
      if (reply) {
        const suggestions = TOPIC_SUGGESTIONS[lang2]?.[topic] ?? TOPIC_SUGGESTIONS[lang2]?.['general_mid'] ?? []
        return { reply, suggestions }
      }
    }
  }
  ...
  const reply = TOPIC_REPLIES[lang2]?.[generalKey] ?? TOPIC_REPLIES.en[generalKey]!
  const suggestions = TOPIC_SUGGESTIONS[lang2]?.[generalKey] ?? []
  return { reply, suggestions }
}
...
` : ''
    ...
    if (apiKey) {
      ...
      const fallback = getSmartFallback(message, userScore, lang)
