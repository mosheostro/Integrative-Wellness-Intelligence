...
export default function ProfilePage() {
  ...
  if (loading) return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '32px 24px', textAlign: 'center', color: 'var(--ink-faint)', fontFamily: 'var(--font-body)' }}>Loading…</div>
  )

  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '32px 24px' }}>
      <div style={{ marginBottom: 36 }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '.68rem', textTransform: 'uppercase', letterSpacing: '.14em', color: 'var(--sage)', marginBottom: 8 }}>◈ Profile</div>
        <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(1.6rem, 3vw, 2.2rem)', fontWeight: 500, letterSpacing: '-.02em', color: 'var(--ink)', margin: 0 }}>Your Profile</h1>
      </div>
      ...
      <form onSubmit={handleSave} style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 'var(--radius-lg)', padding: '28px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <F label="Full name">
            <input value={profile.full_name ?? ''} onChange={e => setProfile(p => ({ ...p, full_name: e.target.value }))} placeholder="Your name" style={inputStyle} />
          </F>
          <F label="Email">
            <input value={profile.email ?? ''} disabled style={{ ...inputStyle, opacity: 0.5, cursor: 'not-allowed' }} />
          </F>
          <F label="Location">
            <input value={profile.location ?? ''} onChange={e => setProfile(p => ({ ...p, location: e.target.value }))} placeholder="City, Country" style={inputStyle} />
          </F>
          <F label="Date of birth">
            <input type="date" value={profile.date_of_birth ?? ''} onChange={e => setProfile(p => ({ ...p, date_of_birth: e.target.value }))} style={inputStyle} />
          </F>
          <F label="Preferred tradition">
          ...
          <F label="Bio">
            <textarea rows={3} value={profile.bio ?? ''} onChange={e => setProfile(p => ({ ...p, bio: e.target.value }))} placeholder="Tell us a little about your wellness journey…" style={{ ...inputStyle, resize: 'vertical', gridColumn: '1 / -1' }} />
          </F>
        </div>
        ...
        <div style={{ marginTop: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {saved && (
            <span style={{ fontFamily: 'var(--font-body)', fontSize: '.85rem', color: 'var(--sage)', fontWeight: 600 }}>✓ Saved</span>
          )}
        </div>
      </form>
    </div>
