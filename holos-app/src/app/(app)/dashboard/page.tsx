          ...
          {latestRecs && latestRecs.length > 0 && (
            <div className="card-depth" style={{ padding: 24, marginBottom: 24 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {(latestRecs as { id: string; rec_id?: string; impact_score: number; title: string; category: string }[]).map((rec, ri) => (
                  <div key={rec.id} className="glass-card" style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: '.875rem', fontWeight: 500, color: 'var(--ink)', marginBottom: 2 }}>{getRecTitle(rec.rec_id, rec.title, locale)}</div>
