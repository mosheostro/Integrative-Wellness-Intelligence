              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                {assessments.map((a, i) => (
                  <Link key={a.id} href={`/results/${a.id}`}
                    ...
                    <span style={{ fontFamily: 'var(--font-body)', fontSize: '.78rem', color: 'var(--sage-deep)', background: 'rgba(122,158,142,.1)', padding: '2px 10px', borderRadius: 100 }}>{TRADITION_LABELS_L[a.framework]?.[locale] ?? a.framework}</span>
                    <span style={{ fontFamily: 'var(--font-body)', fontSize: '.78rem', color: 'var(--ink-soft)' }}>{STATE_LABELS_L[a.wellness_state ?? '']?.[locale] ?? (a.wellness_state ?? '').replace(/_/g, ' ')}</span>
