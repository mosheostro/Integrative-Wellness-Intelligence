...
// Build branching map for fast lookup
...
// Get the next questions given the current answer
export function getNextQuestions(
  ...
  // Direct follow-ups for this specific answer
  for (const q of QUESTION_BANK) {
    if (!q.showIf) continue
    if (q.showIf.questionId === questionId &&
...
// Get initial (unconditional) questions in order
